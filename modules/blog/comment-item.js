...
function CommentItem({ item, ...props }) {
    ...
    return (
        <CommentItemStyled {...props}>
            {/* image */}
            <Image src={img} alt="comment" width={70} height={70} objectFit="cover" placeholder="blur" blurDataURL="/img/bg/loading.WebP" />
            {/* image */}
            {/* content */}
            <div>
                ...
                <span className="font-small text-uppercase d-block mb-2">{date}</span>
                <p>{comment}</p>
                <Link href="/blog/blog-single" passHref>
                    <LinkStyled className="font-small a-reset">
                        <ReplaytIcon className="mr-2" /> replay
                    </LinkStyled>
                </Link>
            </div>
            {/* content */}
