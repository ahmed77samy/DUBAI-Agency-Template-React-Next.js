import {
    TrophyIcon,
    MoneyIcon,
    FlagIcon,
    TrendingIcon,
    SmileIcon,
    RetweetIcon,
} from "components/elements/icons/icons";

export const portfolio_items = [
    {
        id: 1,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/1.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged."
    },
    {
        id: 2,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/2.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 3,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/3.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged."
    },
    {
        id: 4,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/4.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 5,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/5.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 6,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/6.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 7,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/1.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged."
    },
    {
        id: 8,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/2.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 9,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/3.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 10,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/4.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 11,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/5.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged."
    },
    {
        id: 12,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/6.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 13,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/1.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 14,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/2.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 15,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/3.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged."
    },
    {
        id: 16,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/4.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 17,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/5.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged."
    },
    {
        id: 18,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/6.jpg",
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged."
    },
];

export const latest_portfolio_items = portfolio_items.filter((e, i) => i < 6);

export const portfolio_two_items = [
    {
        id: 19,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/7.jpg",
        category: ["brand", "photograph", "graphic"],
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 20,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/8.jpg",
        category: ["design", "motion", "creative"],
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 21,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/9.jpg",
        category: ["portfolio", "design"],
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 22,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/7.jpg",
        category: ["brand", "photograph", "graphic"],
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 23,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/8.jpg",
        category: ["design", "motion", "creative"],
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 24,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/9.jpg",
        category: ["portfolio", "design"],
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 25,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/7.jpg",
        category: ["brand", "photograph", "graphic"],
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 26,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/8.jpg",
        category: ["design", "motion", "creative"],
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
    {
        id: 27,
        name: "Lorem Ipsum is not simply random text.",
        img: "/img/portfolio/9.jpg",
        category: ["portfolio", "design"],
        description:
            "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s,",
    },
];

export const latest_portfolio_two_items = portfolio_two_items.filter((e, i) => i < 3);

export const parts_items = [
    "/img/portfolio/11.jpg",
    "/img/portfolio/12.jpg",
    "/img/portfolio/13.jpg",
    "/img/portfolio/14.jpg",
    "/img/portfolio/15.jpg",
    "/img/portfolio/16.jpg",
];

export const facts_items = [
    {
        fact: "15 awards",
        Icon: TrophyIcon,
    },
    {
        fact: "best seller",
        Icon: MoneyIcon,
    },
    {
        fact: "10 countries",
        Icon: FlagIcon,
    },
    {
        fact: "3 trending",
        Icon: TrendingIcon,
    },
    {
        fact: "46 satisfied",
        Icon: SmileIcon,
    },
    {
        fact: "interested",
        Icon: RetweetIcon,
    },
    {
        fact: "15 awards",
        Icon: TrophyIcon,
    },
    {
        fact: "best seller",
        Icon: MoneyIcon,
    },
];
